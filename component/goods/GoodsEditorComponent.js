...
class RenderBase extends  BaseComponent{
    ...
    insertGoodsDetail(){
        ...
        if(company_name==='ssp'){
            ...
            if(this.state.send_goods_time!==""){
                params["send_goods_time"]=this.state.send_goods_time;
            }
        ...
        return(
            <div>
                ...
                <div style={{marginLeft:100,marginTop:20,display:'flex',alignItems:'center',flexWrap:'wrap'}}>
                    <CheckComponent
                        title="是否商家推荐"
                        visible={this.state.detailBean.is_merchants_recommend}
                        checked={this.state.goodsBean.is_merchants_recommend}
                        onClick={(value)=>{
                            this.state.goodsBean.is_merchants_recommend=value;
                            this.setState({
                                goodsBean:this.state.goodsBean
                            });
                        }}/>
                ...
                <div style={styles.div}>
                    <div style={{width:100,display:'flex',justifyContent:'flex-end',}}>
                        <p1 style={{fontSize:13}}>时间排序</p1>
                    </div>
                    <DateField
                        style={{marginLeft:10}}
                        dateFormat="YYYY-MM-DD HH:mm:ss"
                        value={this.state.sort_time}
                        onChange={(dateString, { dateMoment, timestamp })=>{
                                   this.setState({
                                       sort_time:dateString,
                                   })
                        }}
                    />
                </div>
